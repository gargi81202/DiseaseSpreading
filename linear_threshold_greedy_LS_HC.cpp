...
class LinearThresholdNetwork {
    ...
    pair<set<string>, int> getKBestVaccinationsLocalSearch() {
        auto start = std::chrono::high_resolution_clock::now();
        set<string> uninfectedNodes;
        set_difference(nodes.begin(), nodes.end(), initialInfectedNodes.begin(), initialInfectedNodes.end(),
                inserter(uninfectedNodes, uninfectedNodes.begin()));
        ...
        random_device rd;
        vector<string> kNodes;
        mt19937 gen(rd());
        uniform_int_distribution<> dis(0, availableNodes.size() - 1);

        kNodes = greedySolution;
        for (auto n : kNodes) usedNodes.insert(n);
        int initialSavedSum = 0;
        for (auto& dn : deterministicInstances) {
            set<string> savedNodes = dn.vaccinateNodesAndCheck(set<string> (kNodes.begin(), kNodes.end()));
        ...
        while (updateFound) {
            ...
            for (int i = 0; i < kNodes.size(); i++) {
                set<string> neighbours_n;
                int x = 0;
                for (auto& dn : deterministicInstances) {
                    set<string> res;
                    set<string> two = dn.getOutgoingEdgeNodes(kNodes[i]);
                    set<string> three = dn.getIncomingEdgeNodes(kNodes[i]);
                    set_union(two.begin(), two.end(), three.begin(), three.end(),
                        inserter(res, res.begin()));
                    two = res;
                    res.clear();
                    set_union(neighbours_n.begin(), neighbours_n.end(), two.begin(), two.end(),
                        inserter(res, res.begin()));
                        
                    neighbours_n = res;
                }
                for (auto nbr : neighbours_n) {
                    set<string> newset (kNodes.begin(), kNodes.end());
                    newset.erase(kNodes[i]);
                    newset.insert(nbr);
                    int currSavedSum = 0;
                    for (auto& dn : deterministicInstances) {
                        set<string> savedNodes = dn.vaccinateNodesAndCheck(newset);
                        currSavedSum += savedNodes.size();
                        set<string> intersectionResult;
                    }
                    if (currSavedSum > initialSavedSum) {
                        initialSavedSum = currSavedSum;
                        kNodes.erase(kNodes.begin() + i);
                        i--;
                        kNodes.push_back(nbr);
                        usedNodes.insert(nbr);
                        updateFound = true;
                        break;
                    }
                    else usedNodes.insert(nbr);
                }
            }
        }
        auto end = std::chrono::high_resolution_clock::now();
        ...
        cout << duration.count() << endl;
        cout << (nodes.size() * numInstances - initialSavedSum) << endl;
        return make_pair(set<string> (kNodes.begin(), kNodes.end()), initialSavedSum);       
    }

    set<string> getKBestVaccinationsHillClimbing() {
        auto start = std::chrono::high_resolution_clock::now();
        set<string> uninfectedNodes;
        set_difference(nodes.begin(), nodes.end(), initialInfectedNodes.begin(), initialInfectedNodes.end(),
                inserter(uninfectedNodes, uninfectedNodes.begin()));
        ...
        while (updateFound) {
            updateFound = false;
            for (int i = 0; i < kNodes.size(); i++) {
                set<string> neighbours_n = getNodes();
                neighbours_n.erase(kNodes[i]);
                for (auto nbr : neighbours_n) {
                    set<string> newset (kNodes.begin(), kNodes.end());
                    newset.erase(kNodes[i]);
                    newset.insert(nbr);
        ...
        cout << duration.count() << endl;
        cout << (nodes.size() * numInstances - initialSavedSum) << endl;
        return set<string> (kNodes.begin(), kNodes.end());       
    }

    set<string> getKBestVaccinationsTabuSearch() {
        set<string> uninfectedNodes;
        set_difference(nodes.begin(), nodes.end(), initialInfectedNodes.begin(), initialInfectedNodes.end(),
                inserter(uninfectedNodes, uninfectedNodes.begin()));
        vector<string> availableNodes(uninfectedNodes.begin(), uninfectedNodes.end());
        deque<string> usedNodes;
        set<int> selectedIndices;
        random_device rd;
        vector<string> kNodes;
        mt19937 gen(rd());
        ...
        kNodes = greedySolution;
        for (auto n : kNodes) usedNodes.push_back(n);
        ...
        while (updateFound) {
            ...
            for (int i = 0; i < kNodes.size(); i++) {
                ...
                for (auto nbr : neighbours_n) {
                    if (find(usedNodes.begin(), usedNodes.end(), nbr) != usedNodes.end()) continue;
                    set<string> newset (kNodes.begin(), kNodes.end());
                    newset.erase(kNodes[i]);
                    newset.insert(nbr);
                    int currSavedSum = 0;
                    ...
                    if (currSavedSum >= initialSavedSum) {
                        ...
                        usedNodes.push_back(nbr);  // Add new node to the deque
                        if (usedNodes.size() > numVaccines) {
                            usedNodes.pop_front();  // Remove the oldest node if deque exceeds size
                        }
    ...
    int getOptimumNumInstances() {
        int initialNumInstances = numInstances;
        int left = 1;
        int right = pow(2, int(nodes.size()) - 1);
        while (2*left < right) {
            setNumInstances(getRandomInt(left, 2*left));
            getDeterministicInstances();
            int leftSaved = getKBestVaccinationsLocalSearch().second;
            setNumInstances(getRandomInt(right, 2*right));
            getDeterministicInstances();
            int rightSaved = getKBestVaccinationsLocalSearch().second;
            if (leftSaved >= rightSaved) {
                right = pow(2, (log2(right*left)/2));
            }
            else left = pow(2, (log2(right*left)/2));
        }
        setNumInstances(initialNumInstances);
        return left;
    }
};
...
int main (int argc, char* argv[]) {
    ...
    string filename = "deterministicInstances_" +  to_string(diseaseSpreadingNetwork.getNodes().size()) + "_" + to_string(diseaseSpreadingNetwork.getNumVaccines()) + "_" + to_string(diseaseSpreadingNetwork.getNumInstances()) + ".json";
    diseaseSpreadingNetwork.saveInstancesToJson(filename);
    diseaseSpreadingNetwork.getInitialKNodes();
    set<string> res = diseaseSpreadingNetwork.getKBestVaccinationsLocalSearch().first;
    set<string> res2 = diseaseSpreadingNetwork.getKBestVaccinationsHillClimbing();
